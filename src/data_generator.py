...
def generate_agricultural_data(num_samples=5000, random_seed=42):
    """
    Generates a scientifically-inspired agricultural carbon footprint dataset.
    ...
    # Soil Organic Carbon (%)
    soc = np.clip(
        np.random.normal(loc=2.2, scale=0.8, size=num_samples),
        0.5,
        5.0
    )
    ...
    # Soil pH
    ph = np.clip(
        np.random.normal(loc=6.4, scale=0.7, size=num_samples),
        4.5,
        8.5
    )
    ...
    n_content = np.clip(
        np.random.normal(loc=75, scale=25, size=num_samples),
        10,
        200
    )

    p_content = np.clip(
        np.random.normal(loc=35, scale=15, size=num_samples),
        5,
        100
    )

    k_content = np.clip(
        np.random.normal(loc=65, scale=20, size=num_samples),
        10,
        150
    )
    ...
    # Weather variables
    temperature = np.clip(
        np.random.normal(loc=23.5, scale=5.2, size=num_samples),
        10,
        38
    )

    rainfall = np.clip(
        np.random.normal(loc=850, scale=350, size=num_samples),
        200,
        2000
    )

    humidity = np.clip(
        np.random.normal(loc=65, scale=12, size=num_samples),
        30,
        90
    )

    # ==========================================================
    # 2. FERTILIZER GENERATION
    # ==========================================================
    ...
    e_crop_baseline = np.array(
        [crop_baselines[c] for c in crop_type]
    )

    # Fertilizer emissions
    fert_factors = {
        'Urea': 2.3,
        'Ammonium Nitrate': 2.9,
        'NPK_15-15-15': 1.5,
        'Organic': 0.4,
        'None': 0.0
    }

    e_fertilizer = np.array([
        fert_factors[ft] * fa
        for ft, fa in zip(fertilizer_type, fertilizer_amount)
    ])
    ...
    carbon_footprint = np.round(
        carbon_footprint + noise,
        1
    )
